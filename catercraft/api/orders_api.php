...
$orderQuery = $con->prepare("
    SELECT id, username, payment_id, payment_status, order_status, 
           COALESCE(order_date, created_at, NOW()) AS order_date
    FROM orders
    WHERE username = ?
    ORDER BY id DESC
");
...
while ($o = $result->fetch_assoc()) {
    ...
    $orders[] = [
        ...
        'payment_status' => $o['payment_status'] ?? 'Pending Payment',
